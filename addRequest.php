...
<?php
    if(isset($_POST['req'])){
        ...
        if ($add_book){
            $select_book = "SELECT id FROM book WHERE nama = '$title' AND penulis = '$author' AND penerbit = '$pub'";
            $res = mysqli_query($con, $select_book);
            if($res){
                $book = mysqli_fetch_array($res);
                $book_id = $book['id'];
                $query_req = "INSERT INTO request_book (user_id, book_id, status) VALUES ('$user_id', '$book_id', '$req_status')";
